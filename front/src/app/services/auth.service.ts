...
const storage = {
    ...
    set: (k: string, v : string) => {typeof localStorage != 'undefined'? localStorage.setItem(k, v) : null},

    del: (k: string) => {typeof localStorage != 'undefined' ? localStorage.removeItem(k) : null}
}
...
export class AuthService{
    ...
    login(username : string, password : string):Observable<TokenPair>{
        ...
        return this.http.post<TokenPair>(AUTH_URL, {username, password}).pipe(
            tap(tokens => {
                if(tokens.access){
                    this._access.set(tokens.access);
                    storage.set('access', tokens.access)
                }

                if(tokens.refresh){
                    this._refresh.set(tokens.refresh);
                    storage.set('refresh', tokens.refresh)
                }
            })
        )
    }
    ...
    logout(){
        ...
        storage.del('acess'); storage.del('refresh'); 
    }
}
